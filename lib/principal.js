...
var create = function(callback) {
    ...
    async.series(flow, function(err, params) {
        ...
        if (cli.flags.apiKey) {
            ...
            return callback(new Error("For non-user principals you must provide an API key. ('n2 apikeys ls' shows you all of your API keys.)"));
        }
...
var list = function(type, callback) {
    console.log('list before startSession');
    cli.globals.startSession(function(err, session, user) {
        console.log('list after startSession: ' + err);
        if (err) return callback(err);
