...
var update = function(callback) {
    ...
    cli.globals.startSession(function(err, session, user) {
        ...
        nitrogen.Principal.findById(session, cli.flags.id, function(err, principalFound, accessToken) {
            if (err) return callback(err);
            if (!principalFound) return callback();

            var updates = { };
            if (cli.flags.type)     { updates.type       = cli.flags.type;            }
            if (cli.flags.email)    { updates.email      = cli.flags.email;           }
            if (cli.flags.name)     { updates.name       = cli.flags.name;            }
            if (cli.flags.nickname) { updates.nickname   = cli.flags.nickname;        }
            if (cli.flags.password) { updates.password   = cli.flags.password;        }
            if (cli.flags.tags)     { updates.tags       = cli.flags.tags.split(','); }
            if (cli.flags.apiKey)   { updates.api_key    = cli.flags.apiKey;          } 
            
            // Save the updated principal
            nitrogen.Principal.update(session, principalFound.id, updates, function(err, response, body) {
                if (err) return callback(err);

                if (principalFound.type !== 'user') {
                    if (cli.flags.claim)
                        return sendClaim(cli.flags.claim, callback);
                    else
                        return callback();
                } else {
                    setCredentials(email, password, function(err) {
...
var help = function(callback) {
    cli.utils.displayCommands('Principal Commands', [
        ...
        ['',              '  --id: id of the principal to be updated.'],
        ['',              '  --tags: comma delimited tags for this principal.'],
        ['',              '  --email: email for user principal'],
        ['',              '  --name: name of principal'],
        ['',              '  --password: password for user principal'],
