...
var create = function(callback) {
    ...
    async.series(flow, function(err, params) {
        ...
        if (cli.flags.capabilities) {
            principal.capabilities = cli.flags.capabilities.split(',');
        }

        cli.service.getService(function(err, service) {
            if (err) return callback(err);

            console.log('creating principal: ' + JSON.stringify(principal));
...
var help = function(callback) {
    ...
    cli.log.help('    --capabilities: comma delimited list of messages this principal can act on (eg. cameraCommand).');
