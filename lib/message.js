...
var help = function(callback) {
    ...
    cli.utils.displayCommands('Message Commands', [
        ...
        ['import <file>', 'Imports a message stream from <file>'],
...
var importFile = function(callback) {
    ...
    fs.readFile(filePath, function(err, jsonString) {
        ...
        cli.globals.startSession(function(err, session, user) {
            ...
            async.whilst(
                function() { return start < messages.length },
                function(batchCallback) {
                    var batch = messages.slice(start, 1)
                                        .map(function(batchObject) {
                                            return new nitrogen.Message(batchObject);
                                        });
                    start += 1;
                    console.dir(batch);
                    nitrogen.Message.sendMany(session, batch, batchCallback);
                },
                callback
            );
        });
